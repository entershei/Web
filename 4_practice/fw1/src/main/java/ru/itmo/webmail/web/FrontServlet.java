...
public class FrontServlet extends HttpServlet {
    ...
    private Method getMethod(String nameMethod, Class<?> clazz) throws
            NotFoundException {
        Method method = null;

        for (Class<?> c = clazz; method == null && c != null; c = c.getSuperclass()) {
            for (Method i : c.getDeclaredMethods()) {
                if (i.getName().equals(nameMethod)) {
                    method = i;
                    break;
                }
            }
    ...
    private void processRoute(Route route, HttpServletRequest request, HttpServletResponse response) throws
            NotFoundException, ServletException, IOException {
        Class<?> clazz = route.newClass();

        Method before = getMethod("before", clazz);
        Method method = getMethod(route.getAction(), clazz);
        Method after = getMethod("after", clazz);

        Map<String, Object> view = new HashMap<>();

        callMethod(before, view, request, response, clazz);
        callMethod(method, view, request, response, clazz);
        callMethod(after, view, request, response, clazz);
    ...
    private void callMethod(Method method, Map<String, Object> view, HttpServletRequest request,
                            HttpServletResponse response, Class<?> clazz) throws ServletException, IOException {
        Object page;
        try {
        ...
        try {
            ...
            for (Class i : method.getParameterTypes()) {
                if (i.equals(Map.class)) {
                    parameters.add(view);
                }
                if (i.equals(HttpServletRequest.class)) {
