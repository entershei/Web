...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException, NoSuchAlgorithmException {
        ...
        String uri = urlDecode_to_utf8(request.getUri());
        File file = new File(root, uri);

        if (file.isDirectory()) {
            URI indexURI = file.toURI().resolve("index.html");
            file = Paths.get(indexURI).toFile();
        }
    ...
    private String urlDecode_to_utf8(String url) throws UnsupportedEncodingException {
        return URLDecoder.decode(url, "utf8");
    }

    private String getETag(byte[] body) throws NoSuchAlgorithmException {
        MessageDigest digest = MessageDigest.getInstance("SHA-256");
        return DatatypeConverter.printHexBinary(digest.digest(body));
    }
